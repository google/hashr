...
func TestExtractTarGz(t *testing.T) {
	for _, tc := range []struct {
		tarGzPath          string
		wantExtractedFiles map[string]string
	}{
		{
			tarGzPath: "testdata/targz/dir1/desktop.tar.gz",
			wantExtractedFiles: map[string]string{
		...
		{
			tarGzPath: "testdata/targz/dir1/laptop.tar.gz",
			wantExtractedFiles: map[string]string{
		...
		{
			tarGzPath: "testdata/targz/dir1/server.tar.gz",
			wantExtractedFiles: map[string]string{
		...
		{
			tarGzPath: "testdata/targz/dir2/desktop.tar.gz",
			wantExtractedFiles: map[string]string{
		...
		{
			tarGzPath: "testdata/targz/dir2/laptop.tar.gz",
			wantExtractedFiles: map[string]string{
		...
		{
			tarGzPath: "testdata/targz/dir2/server.tar.gz",
			wantExtractedFiles: map[string]string{
		...
		{
			tarGzPath: "testdata/targz/dir4/desktop.tar.gz",
			wantExtractedFiles: map[string]string{
		...
		{
			tarGzPath: "testdata/targz/dir4/laptop.tar.gz",
			wantExtractedFiles: map[string]string{
		...
		{
			tarGzPath: "testdata/targz/dir4/server.tar.gz",
			wantExtractedFiles: map[string]string{
		...
		{
			tarGzPath: "testdata/targz/dir3/desktop.tar.gz",
			wantExtractedFiles: map[string]string{
		...
		{
			tarGzPath: "testdata/targz/dir3/laptop.tar.gz",
			wantExtractedFiles: map[string]string{
		...
		{
			tarGzPath: "testdata/targz/dir3/server.tar.gz",
			wantExtractedFiles: map[string]string{
		...
		tempDir, err := ioutil.TempDir("", "hashr-")
		if err != nil {
			t.Fatalf("Could not create temp extraction directory(%s): %v", tempDir, err)
		}

		if err := ExtractTarGz(tc.tarGzPath, tempDir); err != nil {
			t.Fatalf("Unexpected error while running ExtractTarGz(%s,%s): %v", tc.tarGzPath, tempDir, err)
		}
		...
		gotExtractedFiles := make(map[string]string)
		for _, file := range files {
			...
			gotExtractedFiles[filename] = fmt.Sprintf("%x", hash)
		}

		if !cmp.Equal(tc.wantExtractedFiles, gotExtractedFiles) {
			t.Errorf("Preprocess() unexpected diff (-want/+got):\n%s", cmp.Diff(tc.wantExtractedFiles, gotExtractedFiles))
		}
	}
}
...
func TestCopyToLocal(t *testing.T) {
	tarGzPath := "testdata/targz/dir1/laptop.tar.gz"
	out, err := CopyToLocal(tarGzPath, "laptop")
	if err != nil {
		t.Fatalf("Unexpected error while running CopyToLocal(): %v", err)
	}

	sha256, err := sha256sum(out)
	if err != nil {
		t.Fatalf("Unexpected error while running sha256sum(): %v", err)
	}

	wantHash := "59f38b7323b022bc57ff1e364476b70bf657658e18667d8805cdf50f256ae4a9"
	gotHash := fmt.Sprintf("%x", sha256)

	if gotHash != wantHash {
		t.Errorf("Sha256 hash of the copied file does match. Want = %s, got = %s", wantHash, gotHash)
	}
